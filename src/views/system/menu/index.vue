    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="按钮类型">
          <el-select v-model="temp.type" class="filter-item" placeholder="Please select">
            <el-option v-for="item in btnTypeList" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="排序" prop="loginName">
          <el-input v-model="temp.sort" />
        </el-form-item>

        <el-form-item label="链接" prop="realName">
          <el-input v-model="temp.href" />
        </el-form-item>
        <el-form-item label="图标" prop="realName">
...
<script>
  ...
  import { baseData } from '@/api/system/menu'
  ...
  export default {
    ...
    methods: {
      getButType(){
        let query={type: "menu_type"}
        basicsApi(baseData.btnTypeS,query).then(response => {
          let result=response.data;
          result.forEach(item=>{
            this.btnTypeList.push({
              value:item.id,
              label:item.label
            })
          })
          console.log(this.btnTypeList);
        })
      },
      getInLevelMuen(){
        let query={userId: store.getters.userId}
        basicsApi(baseData.inLevelS,query).then(response => {
          let result=getTree(response.data)
          this.TreeSelectlist=result;
        })
      },
      getList() {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            basicsApi(baseData.save,formData).then(() => {
      ...
      handleUpdate(row) {
        let result=row;
        const updateData=["id","name","parentId","href","icon","remarks","sort","type","version"]
        let newObj=createNewObj(result,updateData)

        newObj.parentId=[newObj.parentId];
